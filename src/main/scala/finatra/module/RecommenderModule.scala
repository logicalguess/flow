...
import finatra.service.{ALSRecommenderService, FlinkALSRecommenderService, SparkALSRecommenderService, RecommenderService}
...
object RecommenderModule extends TwitterModule {
  ...
  private val recType = flag("rec.type", "spark", "basic or spark or flink")
  ...
  @Singleton
  @Provides
  def recommenderProvider(@Inject() sco: Option[SparkContext], dataProvider: DataProvider):
    RecommenderService = {

    recType() match {
      case REC_TYPE_BASIC => ALSRecommenderService(sco.get, dataProvider)
      case REC_TYPE_SPARK => SparkALSRecommenderService(sco.get, dataProvider)
