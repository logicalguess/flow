...
case class FlinkALSRecommenderService @Inject()(dataProvider: DataProvider) extends RecommenderService {
  ...
  def getRecommendationsForUser(userId: Int, count: Int) = {
    ...
    val mapById: (Int, DataSet[Int]) => DataSet[(Int, Int)] =  {
      (userId: Int, ds: DataSet[Int]) => ds.map((userId, _))
    }

    val predict: (ALS, DataSet[(Int, Int)]) => Array[(Int, Int, Double)] = {
  ...
  def createModel(): ExecutionInfo = {
    ...
    val ratings: DataSet[Rating] = env.fromCollection(dataProvider.getRatings.collect().toList)
...
object FlinkFunctions {
  ...
  val addRandomLongColumn: DataSet[Rating] => DataSet[(Long, Rating)]  = { rs =>
    val rand = new Random()
    rs.map { r => (rand.nextInt(10).toLong, r) }
  }

  val trainingFilter: DataSet[(Long, Rating)] => DataSet[(Int, Int, Double)]  = { rs =>
    rs.filter(x => x._1 <= 3).map(x => x._2).map(r => (r.user, r.product, r.rating))
  }
  //val fn: (RDD[(Long, Rating)]) => RDD[Rating] = trainingFilter _

  def validationFilter(rs: DataSet[(Long, Rating)]): DataSet[(Int, Int, Double)]  = {
    rs.filter(x => x._1 == 4).map(x => x._2).map(r => (r.user, r.product, r.rating))
  }

  def testingFilter(rs: DataSet[(Long, Rating)]): DataSet[(Int, Int, Double)]  = {
    rs.filter(x => x._1 == 5).map(x => x._2).map(r => (r.user, r.product, r.rating))
  }

  def train: (DataSet[(Int, Int, Double)], DataSet[(Int, Int, Double)]) =>  ALS = {
    (validation, training) =>
